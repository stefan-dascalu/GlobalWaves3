...
public final class Admin {
    ...
    public void setSongs(final List<SongInput> songInputList) {
        ...
        adBreakSong = songs.stream().filter(s -> s.matchesName("ad break")).findFirst().get();
    }
    ...
    public List<User> getPremiumUsers() {
        return users.stream().filter(User::isPremium).toList();
    }
    ...
    public List<UserStatistics> getUserStatistics() {
        return new ArrayList<>(users.stream().map(User::getStatistics).toList());
    }
    ...
    public String addNewUser(final CommandInput commandInput) {
        ...
        if (type.equals("user")) {
            ...
            ArrayList<Podcast> hostPodcasts = podcasts.stream()
                    .filter(p -> p.matchesOwner(username))
                    .collect(Collectors.toCollection(ArrayList::new));
            newHost.setPodcasts(hostPodcasts);
            hosts.add(newHost);
        }
    ...
    public String buyMerch(final CommandInput commandInput) {
        ...
        if (page instanceof ArtistPage artistPage) {
            Artist artist = (Artist) artistPage.getUser();
            Optional<Merchandise> merchandise = artist.getMerch().stream()
                    .filter(m -> m.getName().equals(merchName)).findFirst();
            if (merchandise.isEmpty()) {
                return "The merch %s doesn't exist.".formatted(merchName);
            }

            Merchandise merch = merchandise.get();
            user.addMerch(merch);
            artist.increaseMerchRevenue(merch.getPrice());
    ...
    /**
     * Gets the merchandise list of user.
     *
     * @param username the username
     * @return the list of merch
     */
    public List<String> seeMerch(final String username) throws InvalidUsernameException {
        User user = getUser(username);
        ...
        return user.getBoughtMerchandise().stream().map(Merchandise::getName).toList();
    }
    ...
    /**
     * Gets monetization info for artists.
     *
    ...
    public LinkedHashMap<String, MonetizationOutput> endProgram() {
        ...
        return artists.stream().filter(a -> a.getStatistics().getListens() > 0
                        || a.getMerchRevenue() > 0)
                .sorted(Comparator.comparing((Artist a) -> a.getMerchRevenue()
                                + a.getTotalSongRevenue())
                        .reversed()
                        .thenComparing(Artist::getUsername))
                .collect(Collectors.toMap(Artist::getUsername,
                        a -> {
                            String songName = "N/A";
                            Optional<Map.Entry<Song, Double>> hSong = a.getHighestRevenueSong();
                            if (hSong.isPresent()) {
                                songName = hSong.get().getKey().getName();
                            }

                            return new MonetizationOutput(a.getMerchRevenue(),
                                    Math.round(a.getTotalSongRevenue() * 100.0) / 100.0,
                                    rank.getAndIncrement(), songName);
                        },
    ...
    public String adBreak(final CommandInput commandInput) {
        ...
        if (user.getPlayer().getSource() == null) {
            return "%s is not playing any music.".formatted(username);
        }
        user.adBreak(commandInput.getPrice());
    ...
    public List<Notification> getNotifications(final String username) {
        User user = getUser(username);
