...
import app.statistics.MonetizationOutput;
...
public final class Admin {
    ...
    public LinkedHashMap<String, MonetizationOutput> endProgram() {
        ...
        return artists.stream().filter(a -> a.getMonetization() != null)
                .sorted(Comparator.comparing((Artist a) -> a.getMonetization().getMerchRevenue())
                        .reversed()
                        .thenComparing(Artist::getUsername))
                .collect(Collectors.toMap(Artist::getUsername, a -> {
                    MonetizationOutput monetizationOutput = new MonetizationOutput();
                    monetizationOutput.setRanking(rank.getAndIncrement());
                    return monetizationOutput;
